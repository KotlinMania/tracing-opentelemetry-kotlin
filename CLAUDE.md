# Claude Code Project Instructions — tracing-opentelemetry-kotlin

## Project Overview

This is **tracing-opentelemetry-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tracing-opentelemetry`](https://crates.io/crates/tracing-opentelemetry).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tracing-opentelemetry/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tracing-opentelemetry>
